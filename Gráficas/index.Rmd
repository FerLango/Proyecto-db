...
mxstate_choropleth(mapa, num_colors = 1, 
    ...
    ggplotly()
```
Vemos que aproximadamente el 70% de los encuestados tienen 19 o 20 años, esto es debido a que nuestro circulo social esta rodeado de personas de nuestra edad.
#### Género
...
```
Casi el 90% de los encuestados pertenecen a área  y área 3.
# Exploratorias
...
ggplotly(temp)
```
A la mayoría de los estudiantes les preocupa su seguridad digital, lo cual es bueno, ya que hay una intención de informarse respecto a este tema.
...
#### ¿Cuál es tu username en instagram?
```{r}
temp <- 
    informacion %>%
    group_by(username) %>%
...
user <- c("Proporcionó su usuario", "No lo proporcionó")
n <- c(sum(temp$n)-no,no)
temp <-data.frame(user,n)

plot_ly(temp, labels = ~user, values = ~n, type='pie')
```
Podemos ver con que facilidad los usuarios proporcian su información personal.
